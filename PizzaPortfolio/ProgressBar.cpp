...
bool ProgressBar::Init(int x, int y, int width, int height)
{
	...
	_progressBarTop = IMAGEMANAGER->AddImage(TEXT("FrontBar"), TEXT("Image/Fortress/hpBarTop.bmp"), x, y, width, height, true, COLOR_M);
	_progressBarBottom = IMAGEMANAGER->AddImage(TEXT("BackBar"), TEXT("Image/Fortress/hpBarBottom.bmp"), x, y, width, height, true, COLOR_M);
...
void ProgressBar::Render(HDC hdc, int _cameraMoveX, int _cameraMoveY)
{
	_progressBarBottom->Render(hdc, _rcProgress.left + _progressBarBottom->GetWidth() / 2 - _cameraMoveX,
		_y + _progressBarBottom->GetHeight() / 2 - _cameraMoveY,
		0, 0,
		_progressBarBottom->GetWidth(),
		_progressBarBottom->GetHeight());

	_progressBarTop->Render(hdc, _rcProgress.left + _progressBarTop->GetWidth() / 2 - _cameraMoveX,
		_y + _progressBarTop->GetHeight() / 2 - _cameraMoveY,
		0, 0, _width, _progressBarTop->GetHeight());
}
