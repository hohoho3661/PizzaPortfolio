...
void Image::AniRenderCenterAlpha(HDC hdc, int centerX, int centerY, Animation * ani, BYTE alpha)
{
	...
	if (_trans)
	{
		BitBlt(_blendImage->hMemDC, 0, 0,
			...
			hdc, centerX - GetFrameWidth() / 2, centerY - GetFrameHeight() / 2, SRCCOPY);
		...
		GdiAlphaBlend(hdc, centerX - GetFrameWidth() / 2, centerY - GetFrameHeight() / 2,
			_imageInfo->frameWidth,
			_imageInfo->frameHeight,
...
void Image::FrameRenderCenterPoint(HDC hdc, POINT pt, int currentframeX)
{
	_imageInfo->currentframeX = currentframeX;
	...
	if (_trans)
	{
		...
		(
			hdc,
			pt.x - GetFrameWidth() / 2,
			pt.y - GetFrameHeight() / 2,
			_imageInfo->frameWidth,
			_imageInfo->frameHeight,
			_imageInfo->hMemDC,
			currentframeX * _imageInfo->frameWidth,
